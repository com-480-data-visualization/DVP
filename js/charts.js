...
var REGION_COLOR = {
    'Europe':'#4a90e2','South America':'#ff4d6d','CONCACAF':'#f5c842','N&C America':'#f5c842',
    'Africa':'#ff9f43','Asia':'#3dd6c0','Oceania':'#a29bfe',
...
// ── Tooltip ────────────────────────────────────────────────────
function initTip() { TIP = document.getElementById('tooltip'); }
function tip(html, e) {
    if (!TIP) return;
    TIP.innerHTML = html;
...
function initData() {
    if (window.BUNDLE) {
        DATA = window.BUNDLE.data; RACE_DATA = window.BUNDLE.race;
        CHORD_DATA = window.BUNDLE.chord; STREAM_DATA = window.BUNDLE.stream;
        initCharts();
    } else {
        Promise.all([
            ...
            DATA=results[0]; RACE_DATA=results[1]; CHORD_DATA=results[2]; STREAM_DATA=results[3];
            initCharts();
...
function initCharts() {
    if (!DATA || typeof d3 === 'undefined') { console.error('D3 or data not available'); return; }
    var draws = [
...
function drawTimelineGrowth() {
    ...
    var W = elW(el,540)-M.left-M.right, H = 320-M.top-M.bottom;
    var svg = d3.select(el).append('svg').attr('width',W+M.left+M.right).attr('height',H+M.top+M.bottom).style('display','block');
    var g = svg.append('g').attr('transform','translate('+M.left+','+M.top+')');
    var cum = 0;
    var series = DATA.timeline.map(function(d){ cum+=d.count; return {year:d.year,count:cum}; });
    ...
    var ag = defs.append('linearGradient').attr('id','tlGrad2').attr('x1','0%').attr('x2','0%').attr('y1','0%').attr('y2','100%');
    ...
    var path = g.append('path').datum(series).attr('fill','none').attr('stroke',C.gold).attr('stroke-width',2.5).attr('d',line);
    var len = path.node().getTotalLength();
    path.attr('stroke-dasharray',len).attr('stroke-dashoffset',len).transition().duration(2200).ease(d3.easeCubicOut).attr('stroke-dashoffset',0);
    ...
    [{year:1971,label:'Ban lifts',dy:-28},{year:1991,label:'First WC',dy:-28},{year:2015,label:'24 nations',dy:-28}].forEach(function(ev){
        ...
        g.append('line').attr('x1',ex).attr('y1',ey).attr('x2',ex).attr('y2',ey+ev.dy).attr('stroke','rgba(255,255,255,0.3)').attr('stroke-width',1).attr('stroke-dasharray','3,2');
        g.append('circle').attr('cx',ex).attr('cy',ey).attr('r',6).attr('fill',C.rose).attr('stroke',C.gold).attr('stroke-width',2);
        g.append('text').attr('x',ex).attr('y',ey+ev.dy-6).attr('text-anchor','middle').attr('fill','rgba(255,255,255,0.7)').attr('font-size','10px').text(ev.label);
    });
    g.append('g').attr('class','axis').attr('transform','translate(0,'+H+')').call(d3.axisBottom(x).tickFormat(d3.format('d')).ticks(6));
...
function initRaceChart() { if(!RACE_DATA) return; raceYear=1956; drawRaceFrame(1956); }
...
function drawRaceFrame(year) {
    ...
    var teams=RACE_DATA.teams, allData=RACE_DATA.data;
    var frame = teams.map(function(t){
        var arr=allData[t]; var entry=arr?arr.find(function(d){return d.year===year;}):null;
        return {team:t,wins:entry?entry.wins:0};
    }).sort(function(a,b){return b.wins-a.wins;});
    var M={top:8,right:80,bottom:10,left:155};
    var W=elW(el,860)-M.left-M.right, n=Math.min(15,frame.length), rowH=32, svgH=n*rowH+M.top+M.bottom;
    var svg=d3.select(el).select('svg');
    if(svg.empty()){ svg=d3.select(el).append('svg').style('display','block'); svg.append('g').attr('class','race-g').attr('transform','translate('+M.left+','+M.top+')'); }
    svg.attr('width',W+M.left+M.right).attr('height',svgH);
    var g=svg.select('.race-g'), topF=frame.slice(0,n), maxW=Math.max(1,frame[0].wins);
    var xS=d3.scaleLinear().domain([0,maxW]).range([0,W]), dur=200, barH=24;
    var bars=g.selectAll('.rbar').data(topF,function(d){return d.team;});
    bars.enter().append('rect').attr('class','rbar').attr('x',0).attr('y',function(d,i){return i*rowH;}).attr('height',barH)
        .attr('fill',function(d){var t=DATA.top_teams.find(function(tt){return tt.team===d.team;}); return REGION_COLOR[t?t.region:'Other']||C.muted;})
        .attr('rx',2).attr('width',0)
        .merge(bars).transition().duration(dur).attr('y',function(d,i){return i*rowH;}).attr('width',function(d){return xS(d.wins);});
    bars.exit().remove();
    var labs=g.selectAll('.rlabel').data(topF,function(d){return d.team;});
    labs.enter().append('text').attr('class','rlabel').attr('x',-6).attr('text-anchor','end').attr('fill',C.text).attr('font-size','12px').attr('font-family',"'DM Sans',sans-serif").attr('font-weight','500')
        .merge(labs).transition().duration(dur).attr('y',function(d,i){return i*rowH+barH/2+4;}).text(function(d){return d.team;});
    labs.exit().remove();
    var vals=g.selectAll('.rval').data(topF,function(d){return d.team;});
    vals.enter().append('text').attr('class','rval').attr('fill',C.gold).attr('font-size','12px').attr('font-weight','700').attr('font-family',"'Bebas Neue',sans-serif")
        .merge(vals).transition().duration(dur).attr('x',function(d){return xS(d.wins)+5;}).attr('y',function(d,i){return i*rowH+barH/2+4;}).text(function(d){return d.wins;});
    vals.exit().remove();
    var ranks=g.selectAll('.rrank').data(topF,function(d){return d.team;});
    ranks.enter().append('text').attr('class','rrank').attr('fill',C.dim).attr('font-size','10px').attr('font-family',"'DM Sans',sans-serif")
        .merge(ranks).transition().duration(dur).attr('x',-M.left+4).attr('y',function(d,i){return i*rowH+barH/2+4;}).text(function(d,i){return '#'+(i+1);});
    ranks.exit().remove();
}

window.toggleRace = function() {
    if(raceRunning){ clearInterval(raceTimer); raceRunning=false; document.getElementById('race-btn-label').textContent='▶ Play'; }
    else {
        if(raceYear>=2025) raceYear=1956;
        raceRunning=true; document.getElementById('race-btn-label').textContent='⏸ Pause';
        var speed=parseInt(document.getElementById('race-speed').value), delay=Math.max(50,Math.round(400/speed));
        raceTimer=setInterval(function(){ raceYear++; drawRaceFrame(raceYear); if(raceYear>=2025){clearInterval(raceTimer);raceRunning=false;document.getElementById('race-btn-label').textContent='▶ Play';} },delay);
    }
};
window.resetRace = function() { clearInterval(raceTimer); raceRunning=false; raceYear=1956; document.getElementById('race-btn-label').textContent='▶ Play'; drawRaceFrame(1956); };
...
function buildTeamFirstYear() {
    var fy={};
    DATA.timeline.forEach(function(entry){ entry.teams.forEach(function(t){ if(!fy[t]) fy[t]=entry.year; }); });
    return fy;
}
...
function getCountryStatus(geoName,year) {
    var gk=resolveCountry(geoName), keys=Object.keys(teamFirstYear);
    for(var i=0;i<keys.length;i++){
        var team=keys[i], tk=resolveCountry(team);
...
function drawWorldMap() {
    ...
    fetch('https://cdn.jsdelivr.net/npm/world-atlas@2/countries-110m.json')
        ...
        .then(function(world){ window._worldGeo=world; renderMapYear(mapYear); })
        .catch(function(){ mapSvg.append('text').attr('x',W/2).attr('y',H/2).attr('text-anchor','middle').attr('fill',C.muted).attr('font-size','14px').text('World map requires internet connection'); });
}
...
window.updateMapYear=function(yr){ mapYear=parseInt(yr); renderMapYear(mapYear); };
window.toggleMapAnimation=function(){
    var btn=document.getElementById('map-play-btn');
    if(mapAnimRunning){clearInterval(mapAnimTimer);mapAnimRunning=false;btn.textContent='▶ Play';}
    else{
...
function drawChordDiagram() {
    ...
    el.innerHTML='';
    var size=Math.min(elW(el,660),640), innerR=size*0.36, outerR=innerR+22;
    var teams=CHORD_DATA.teams, matrix=CHORD_DATA.matrix;
    var svg=d3.select(el).append('svg').attr('width',size).attr('height',size).style('display','block').style('margin','0 auto').append('g').attr('transform','translate('+(size/2)+','+(size/2)+')');
    ...
    svg.append('g').selectAll('path').data(chord).join('path').attr('class','chord-ribbon')
        .attr('d',ribbon).attr('fill',function(d){return colorScale(teams[d.source.index]);})
        .attr('stroke',function(d){return d3.color(colorScale(teams[d.source.index])).darker();}).attr('opacity',0.65)
        .on('mousemove',function(e,d){ var t1=teams[d.source.index],t2=teams[d.target.index]; tip('<div class="tt-title">'+t1+' vs '+t2+'</div>'+t1+': <span class="tt-val">'+d.source.value+' wins</span><br>'+t2+': <span class="tt-val">'+d.target.value+' wins</span>',e); })
        .on('mouseleave',hideTip);
    var groups=svg.append('g').selectAll('g').data(chord.groups).join('g');
    groups.append('path').attr('d',arcGen).attr('fill',function(d){return colorScale(teams[d.index]);}).attr('stroke','rgba(0,0,0,0.3)').attr('opacity',0.9)
        .on('mousemove',function(e,d){ tip('<div class="tt-title">'+teams[d.index]+'</div>Wins vs top rivals: <span class="tt-val">'+d3.sum(matrix[d.index])+'</span>',e); svg.selectAll('.chord-ribbon').attr('opacity',function(c){return(c.source.index===d.index||c.target.index===d.index)?0.9:0.06;}); })
        .on('mouseleave',function(){ hideTip(); svg.selectAll('.chord-ribbon').attr('opacity',0.65); });
    groups.each(function(d){ d.angle=(d.startAngle+d.endAngle)/2; });
    groups.append('text').attr('dy','0.35em')
        .attr('transform',function(d){return 'rotate('+(d.angle*180/Math.PI-90)+') translate('+(outerR+12)+') '+(d.angle>Math.PI?'rotate(180)':'');})
    ...
    if(legEl){ legEl.innerHTML=''; teams.forEach(function(t,i){ var div=document.createElement('div'); div.className='chord-leg-item'; div.innerHTML='<div class="chord-leg-swatch" style="background:'+CHORD_COLORS[i]+'"></div>'+t; legEl.appendChild(div); }); }
}
...
function drawStreamgraph() {
    ...
    var W=elW(el,860)-M.left-M.right, H=380-M.top-M.bottom;
    var svg=d3.select(el).append('svg').attr('width',W+M.left+M.right).attr('height',H+M.top+M.bottom).style('display','block');
    var g=svg.append('g').attr('transform','translate('+M.left+','+M.top+')');
    ...
    g.append('g').attr('class','grid').call(d3.axisLeft(y).ticks(5).tickSize(-W).tickFormat('')).selectAll('line').style('stroke','rgba(255,255,255,0.06)');
    g.select('.grid .domain').remove();
    var area=d3.area().x(function(d){return x(d.data.year);}).y0(function(d){return y(d[0]);}).y1(function(d){return y(d[1]);}).curve(d3.curveBasis);
    g.selectAll('.spath').data(stacked).join('path').attr('class','spath')
        .attr('d',area).attr('fill',function(d){return streamColors[d.key]||C.muted;}).attr('opacity',0.82)
        .on('mousemove',function(e,d){
            var yr=Math.round(x.invert(d3.pointer(e,g.node())[0]));
            var pt=data.find(function(r){return r.year===yr;}), val=pt?pt[d.key]:0;
    ...
    if(legEl){ legEl.innerHTML=''; regions.forEach(function(r){ var d2=document.createElement('div'); d2.className='stream-leg-item'; d2.innerHTML='<div class="stream-leg-swatch" style="background:'+(streamColors[r]||C.muted)+'"></div>'+r; legEl.appendChild(d2); }); }
}
...
function drawEconomicsChart() {
    ...
    var W=elW(el,780)-M.left-M.right, H=360-M.top-M.bottom;
    var svg=d3.select(el).append('svg').attr('width',W+M.left+M.right).attr('height',H+M.top+M.bottom).style('display','block');
    var g=svg.append('g').attr('transform','translate('+M.left+','+M.top+')');
    ...
    ['men','women'].forEach(function(k,i){
        var gr=defs.append('linearGradient').attr('id','eg2'+k).attr('x1','0%').attr('x2','0%').attr('y1','0%').attr('y2','100%');
    ...
    g.selectAll('.bmen').data(wc).join('rect').attr('class','bmen')
        ...
        .on('mousemove',function(e,d){tip('<div class="tt-title">'+d.year+' Men\'s WC</div>Prize: <span class="tt-val">$'+(d.men_prize/1e6).toFixed(0)+'M</span>',e);}).on('mouseleave',hideTip);
    g.selectAll('.bwom').data(wc).join('rect').attr('class','bwom')
        .attr('x',function(d){return x(d.year)+xo+2;}).attr('y',function(d){return d.women_prize>0?y(d.women_prize):H-3;})
        .attr('width',xo-2).attr('height',function(d){return d.women_prize>0?H-y(d.women_prize):3;})
        .attr('fill','url(#eg2women)').attr('rx',2)
        .on('mousemove',function(e,d){tip('<div class="tt-title">'+d.year+' Women\'s WC</div>Prize: <span class="tt-val">$'+(d.women_prize>0?(d.women_prize/1e6).toFixed(0)+'M':'0')+'</span><br>Attendance: '+d.attendance.toLocaleString(),e);}).on('mouseleave',hideTip);
    wc.filter(function(d){return d.women_prize===0;}).forEach(function(d){
        g.append('text').attr('x',x(d.year)+xo+xo/2).attr('y',H-5).attr('text-anchor','middle').attr('fill',C.rose).attr('font-size','9px').text('$0');
    });
    var last=wc[wc.length-1];
    g.append('text').attr('x',x(last.year)+x.bandwidth()/2).attr('y',y(last.men_prize)-8).attr('text-anchor','middle').attr('fill',C.rose).attr('font-size','11px').attr('font-weight','600').text('89% gap');
...
function drawBubbleChart() {
    ...
    var nodes=scorers.map(function(d){ return Object.assign({},d,{r:rScale(d.goals)+9,x:W/2+(Math.random()-0.5)*200,y:H/2+(Math.random()-0.5)*200}); });
    var sim=d3.forceSimulation(nodes).force('x',d3.forceX(W/2).strength(0.05)).force('y',d3.forceY(H/2).strength(0.05)).force('collide',d3.forceCollide(function(d){return d.r+4;}).strength(0.85)).force('charge',d3.forceManyBody().strength(-8));
    var bubbleG=svg.selectAll('.bub').data(nodes).join('g').attr('class','bub');
    bubbleG.append('circle').attr('r',function(d){return d.r;}).attr('fill',function(d){return colorOrd(d.country);}).attr('opacity',0.84).attr('stroke','rgba(0,0,0,0.3)').attr('stroke-width',1)
        .on('mousemove',function(e,d){ tip('<div class="tt-title">'+d.scorer+'</div><span class="tt-sub">'+d.country+'</span><br>Goals: <span class="tt-val">'+d.goals+'</span><br>Career: '+d.first_year+'–'+d.last_year+'<br>Penalties: '+d.penalties,e); })
        .on('mouseleave',hideTip);
    bubbleG.append('text').attr('text-anchor','middle').attr('dy','0.35em').attr('fill','rgba(255,255,255,0.95)').attr('pointer-events','none')
        .attr('font-size',function(d){return Math.max(7,Math.min(d.r*0.35,13))+'px';}).attr('font-family',"'DM Sans',sans-serif").attr('font-weight','600')
        .text(function(d){return d.r>22?d.scorer.split(' ').pop():'';});
    bubbleG.append('text').attr('text-anchor','middle').attr('dy',function(d){return d.r>22?'1.6em':'0.35em';})
    ...
    clegG.append('text').attr('x',0).attr('y',0).attr('fill','#b8c4d0').attr('font-size','9px').attr('letter-spacing','0.08em').text('COUNTRY');
    countries.slice(0,8).forEach(function(c,i){
        clegG.append('circle').attr('cx',6).attr('cy',14+i*15).attr('r',5).attr('fill',colorOrd(c)).attr('opacity',0.85);
        clegG.append('text').attr('x',15).attr('y',18+i*15).attr('fill','#c8ccd8').attr('font-size','9px').text(c);
    });
}
...
function drawRadialClock() {
    ...
    if(!el) return;
    el.innerHTML='';
    var W=elW(el,400), H=Math.min(W,400), cx=W/2, cy=H/2;
    ...
    [0.25,0.5,0.75,1].forEach(function(t){ g.append('circle').attr('r',innerR+(maxR-innerR)*t).attr('fill','none').attr('stroke','rgba(255,255,255,0.06)').attr('stroke-width',1); });
    var arc=d3.arc().innerRadius(innerR).outerRadius(function(d){return rScale(d.count);}).startAngle(function(d,i){return i*sliceAngle-Math.PI/2;}).endAngle(function(d,i){return (i+1)*sliceAngle-Math.PI/2;}).padAngle(0.01).cornerRadius(2);
    g.selectAll('.rseg').data(bins).join('path').attr('class','rseg').attr('d',arc)
        .attr('fill',function(d){ if(d.minute_bin>=40&&d.minute_bin<=50) return C.rose; if(d.minute_bin>=80) return C.gold; return C.teal; })
        .attr('opacity',0.82)
    ...
    ['0\u2032','22\u2032','45\u2032','67\u2032','90\u2032'].forEach(function(label,i){
        ...
        g.append('text').attr('x',Math.cos(angle)*(maxR+16)).attr('y',Math.sin(angle)*(maxR+16)).attr('text-anchor','middle').attr('dy','0.35em').attr('fill',C.muted).attr('font-size','10px').text(label);
    });
    g.append('text').attr('text-anchor','middle').attr('dy','-0.2em').attr('fill',C.gold).attr('font-family',"'Bebas Neue',sans-serif").attr('font-size','1.2rem').text('GOALS');
    g.append('text').attr('text-anchor','middle').attr('dy','1.2em').attr('fill',C.muted).attr('font-size','9px').text('by minute');
}
...
function drawActivityChart() {
    ...
    var W=elW(el,500)-M.left-M.right, H=170-M.top-M.bottom;
    var svg=d3.select(el).append('svg').attr('width',W+M.left+M.right).attr('height',H+M.top+M.bottom).style('display','block');
    var g=svg.append('g').attr('transform','translate('+M.left+','+M.top+')');
    ...
    g.selectAll('.abar').data(coverage).join('rect').attr('class','abar')
        ...
        .attr('fill',function(d){return d.has_data?C.gold:'rgba(255,255,255,0.12)';}).attr('rx',1)
        ...
        .on('mouseleave',hideTip);
    g.append('g').attr('class','axis').attr('transform','translate(0,'+H+')').call(d3.axisBottom(x).tickValues(coverage.filter(function(d,i){return i%8===0;}).map(function(d){return d.year;})).tickFormat(d3.format('d')));
...
function drawTeamsRanking() {
    ...
    if(currentFilter==='top20') teams=teams.slice(0,20);
        ...
        teams=teams.filter(function(t){
            return ['Asia','Africa','Oceania'].includes(t.region)||['Japan','Australia','Nigeria','South Korea','New Zealand','Colombia','Jamaica'].includes(t.team);
        }).slice(0,15);
    } else { teams=teams.slice(0,25); }
    var M={top:10,right:80,bottom:10,left:155};
    var W=elW(el,860)-M.left-M.right, rowH=28, svgH=teams.length*rowH+20;
    ...
    g.selectAll('.rkbar').data(teams).join('rect').attr('class','rkbar')
        .attr('y',function(d,i){return i*rowH;}).attr('x',0).attr('width',0).attr('height',rowH-5)
        ...
        .on('mouseleave',hideTip)
        .transition().duration(500).delay(function(d,i){return i*15;}).attr('width',function(d){return x(d.wins);});
    g.selectAll('.rkname').data(teams).join('text').attr('class','rkname')
        .attr('x',-8).attr('text-anchor','end').attr('y',function(d,i){return i*rowH+(rowH-5)/2+4;})
    ...
    g.selectAll('.rkval').data(teams).join('text').attr('class','rkval')
        .attr('y',function(d,i){return i*rowH+(rowH-5)/2+4;}).attr('fill',C.gold).attr('font-size','12px').attr('font-weight','700').attr('font-family',"'Bebas Neue',sans-serif")
        .attr('x',0).attr('opacity',0)
        .transition().duration(500).delay(function(d,i){return i*15;}).attr('x',function(d){return x(d.wins)+6;}).attr('opacity',1)
        .tween('text',function(d){ var nd=this; return function(t){nd.textContent=Math.round(d.wins*t);}; });
    g.selectAll('.rkreg').data(teams).join('text').attr('class','rkreg')
        .attr('x',-M.left+4).attr('y',function(d,i){return i*rowH+(rowH-5)/2+4;})
        .attr('fill',function(d){return REGION_COLOR[d.region]||C.muted;}).attr('font-size','9px').attr('font-weight','600')
        .text(function(d){return d.region?d.region.substring(0,5):'';});
}

/* ============================================================
   11. SCATTER CHART — collision-avoided labels
   ============================================================ */
function drawScatterChart() {
    ...
    var W=elW(el,860)-M.left-M.right, H=420-M.top-M.bottom;
    var svg=d3.select(el).append('svg').attr('width',W+M.left+M.right).attr('height',H+M.top+M.bottom).style('display','block');
    var g=svg.append('g').attr('transform','translate('+M.left+','+M.top+')');
    ...
    g.append('line').attr('x1',0).attr('x2',W).attr('y1',y(50)).attr('y2',y(50)).attr('stroke','rgba(255,255,255,0.09)').attr('stroke-dasharray','4,3');
    g.append('line').attr('x1',x(200)).attr('x2',x(200)).attr('y1',0).attr('y2',H).attr('stroke','rgba(255,255,255,0.09)').attr('stroke-dasharray','4,3');
    [['Dominant',W-6,14,'end'],['Rising',4,14,'start'],['Experienced',W-6,H-6,'end'],['Developing',4,H-6,'start']].forEach(function(q){
        g.append('text').attr('x',q[0]).attr('y',q[1]).attr('text-anchor',q[3]).attr('fill','rgba(255,255,255,0.1)').attr('font-size','10px').text(q[2]);
    });
    g.selectAll('.sdot2').data(teams).join('circle').attr('class','sdot2')
        .attr('cx',function(d){return x(d.matches);}).attr('cy',function(d){return y(d.win_rate);}).attr('r',function(d){return r(d.goals_for);})
        .attr('fill',function(d){return REGION_COLOR[d.region]||C.muted;}).attr('stroke','rgba(0,0,0,0.4)').attr('stroke-width',1.2).attr('opacity',0.85)
    ...
    for(var iter=0;iter<40;iter++){
        for(var i=0;i<labelData.length;i++){
            for(var j=i+1;j<labelData.length;j++){
                var dx=labelData[i].tx-labelData[j].tx, dy=labelData[i].ty-labelData[j].ty;
                var dist=Math.sqrt(dx*dx+dy*dy), minDist=28;
                if(dist<minDist&&dist>0){
                    var push=(minDist-dist)/2;
                    labelData[i].tx+=(dx/dist)*push; labelData[i].ty+=(dy/dist)*push*0.8;
                    labelData[j].tx-=(dx/dist)*push; labelData[j].ty-=(dy/dist)*push*0.8;
                }
            }
    ...
    labelData.forEach(function(ld){
        var ox=x(ld.team.matches), oy=y(ld.team.win_rate), rr=r(ld.team.goals_for);
        var dx=ld.tx-ox, dy=ld.ty-oy, len=Math.sqrt(dx*dx+dy*dy);
        if(len>rr+6){ labG.append('line').attr('x1',ox+dx/len*rr).attr('y1',oy+dy/len*rr).attr('x2',ld.tx-4).attr('y2',ld.ty-2).attr('stroke','rgba(255,255,255,0.15)').attr('stroke-width',0.7); }
        labG.append('text').attr('x',ld.tx).attr('y',ld.ty).attr('fill','#e0e4ec').attr('font-size','10px').attr('font-family',"'DM Sans',sans-serif").text(ld.team.team);
    });
    ...
    g.append('text').attr('x',W/2).attr('y',H+42).attr('text-anchor','middle').attr('fill','#b8c0cc').attr('font-size','11px').text('Total Matches Played');
    g.append('text').attr('transform','rotate(-90)').attr('y',-46).attr('x',-H/2).attr('text-anchor','middle').attr('fill','#b8c0cc').attr('font-size','11px').text('Win Rate (%)');
}
...
function populateTeamSelects() {
    var allTeams=DATA.all_teams.filter(function(t){ return DATA.top_teams.some(function(tt){return tt.team===t;}); }).sort();
    ['team1-select','team2-select'].forEach(function(id){
        var sel=document.getElementById(id);
        if(!sel) return;
        sel.innerHTML='<option value="">— Choose —</option>';
        allTeams.forEach(function(t){ var opt=document.createElement('option'); opt.value=t; opt.textContent=t; sel.appendChild(opt); });
    });
    var s1=document.getElementById('team1-select'), s2=document.getElementById('team2-select');
...
window.updateComparison=function(){
    ...
    var team1=DATA.top_teams.find(function(t){return t.team===t1;}), team2=DATA.top_teams.find(function(t){return t.team===t2;});
    if(!team1||!team2) return;
    var h2hKey=t1+'|'+t2, h2hKeyR=t2+'|'+t1, h2h=DATA.head_to_head[h2hKey]||DATA.head_to_head[h2hKeyR];
    var el=document.getElementById('comparison-result');
    if(!el) return;
    el.innerHTML='';
    var compDiv=document.createElement('div'); compDiv.className='comp-grid'; el.appendChild(compDiv);
    function makeCol(team,cls){
        var col=document.createElement('div'); col.className='comp-stats-col '+cls;
        var alignStyle=cls==='t2'?' style="text-align:right"':'';
        col.innerHTML='<div class="comp-team-header '+cls+'">'+team.team+'</div>';
        [['Wins',team.wins],['Win Rate',team.win_rate+'%'],['Matches',team.matches],['Goals For',team.goals_for],['Goals/Match',team.goals_per_match]].forEach(function(item){
    ...
    var radarWrap=document.createElement('div'); radarWrap.className='comp-radar-wrap'; radarWrap.id='comp-radar-wrap'; compDiv.appendChild(radarWrap);
    compDiv.appendChild(makeCol(team2,'t2'));
    drawRadarChart(team1,team2,radarWrap);
    if(h2h){
        var box=document.createElement('div'); box.className='h2h-box';
        var w1=DATA.head_to_head[h2hKey]?h2h.w1:h2h.w2, w2=DATA.head_to_head[h2hKey]?h2h.w2:h2h.w1;
        box.innerHTML='<div class="h2h-title">Head-to-Head Record ('+h2h.matches+' matches)</div><div class="h2h-nums"><div><div class="h2h-big" style="color:var(--gold)">'+w1+'</div><div class="h2h-label">'+t1+' wins</div></div><div class="h2h-vs">·</div><div><div class="h2h-big" style="color:var(--text-muted)">'+h2h.draws+'</div><div class="h2h-label">Draws</div></div><div class="h2h-vs">·</div><div><div class="h2h-big" style="color:var(--teal)">'+w2+'</div><div class="h2h-label">'+t2+' wins</div></div></div>';
        el.appendChild(box);
    }
};
...
function drawRadarChart(team1,team2,container){
    ...
    [0.25,0.5,0.75,1].forEach(function(lv){
        var pts=dims.map(function(d,i){ var a=angle*i-Math.PI/2; return [Math.cos(a)*R*lv,Math.sin(a)*R*lv]; });
        g.append('polygon').attr('points',pts.map(function(p){return p.join(',');}).join(' ')).attr('fill','none').attr('stroke','rgba(255,255,255,0.1)').attr('stroke-width',1);
    });
    dims.forEach(function(d,i){
        var a=angle*i-Math.PI/2;
        g.append('line').attr('x1',0).attr('y1',0).attr('x2',Math.cos(a)*R).attr('y2',Math.sin(a)*R).attr('stroke','rgba(255,255,255,0.12)').attr('stroke-width',1);
        g.append('text').attr('x',Math.cos(a)*(R+16)).attr('y',Math.sin(a)*(R+16)).attr('text-anchor','middle').attr('dy','0.35em').attr('fill','#b8c4d0').attr('font-size','9px').text(d.label);
    });
    [{team:team1,color:'#f5c842'},{team:team2,color:'#3dd6c0'}].forEach(function(item){
        var pts=dims.map(function(d,i){ var a=angle*i-Math.PI/2; var val=(item.team[d.key]||0)/d.max; return [Math.cos(a)*R*val,Math.sin(a)*R*val]; });
        g.append('polygon').attr('points',pts.map(function(p){return p.join(',');}).join(' ')).attr('fill',item.color).attr('fill-opacity',0.15).attr('stroke',item.color).attr('stroke-width',2).attr('stroke-opacity',0.9);
        g.selectAll(null).data(pts).join('circle').attr('cx',function(d){return d[0];}).attr('cy',function(d){return d[1];}).attr('r',3).attr('fill',item.color);
    });
}